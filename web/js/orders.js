...
function setupWebSocket() {
    try {
        eel._websocket = new WebSocket('ws://localhost:8090/eel?page=' + window.location.pathname.split('/').pop());
...
async function saveOrder() {
    ...
    try {
        ...
        if (editingOrderId) {
            ...
            const qrLink = `http://localhost:8090/mark_as_completed?order_id=${qrFilename}`;
